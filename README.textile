...
Using the above example, we have the following.  [Editor's note: See the wiki home page for a legible example of this table: "http://github.com/clr/map_restfully/wikis/home":http://github.com/clr/map_restfully/wikis/home  as something is buggy with github's textile parser here.]
...
<table>
	...
	<tr>
		<td>nap_path( @nap, :method =&gt; :post )</td>
		<td>nap, nap.html, nap/1, nap/1.html</td>
		<td>NapsController#post</td>
		<td>@nap</td>
	</tr>
	<tr>
		<td>nap_path( @nap, :method =&gt; :put )</td>
		<td>nap, nap.html, nap/1, nap/1.html</td>
		<td>NapsController#put</td>
		<td>@nap</td>
	</tr>
	<tr>
		<td>nap_path( @nap, :method =&gt; :delete )</td>
		<td>nap, nap.html, nap/1, nap/1.html</td>
		<td>NapsController#delete</td>
		<td>@nap</td>
	</tr>
	<tr>
		<td>naps_path( @naps )</td>
		<td>naps, naps.html, naps/1,2,3, naps/1,2,3.html</td>
		<td>NapsController#gets</td>
		<td>@naps</td>
	</tr>
	<tr>
		<td>naps_path( @nap, :method =&gt; :post )</td>
		<td>naps, naps.html, naps/1,2,3, naps/1,2,3.html</td>
		<td>NapsController#posts</td>
		<td>@naps</td>
	</tr>
	<tr>
		<td>naps_path( @nap, :method =&gt; :put )</td>
		<td>naps, naps.html, naps/1,2,3, naps/1,2,3.html</td>
		<td>NapsController#puts</td>
		<td>@naps</td>
	</tr>
	<tr>
		<td>naps_path( @nap, :method =&gt; :delete )</td>
		<td>naps, naps.html, naps/1,2,3, naps/1,2,3.html</td>
...
Note the 's' on the end of the controller method names in the plural case.  The above are only examples.  The paths take the usual formats .xml, .js, etc.  The id number in the singular case and the ids in the plural case are optional.  The number 0 or no number at all in the singular case will return a new ActiveRecord object for the instance variable.


As a default, RestfulController will attempt to prepopulate the instance variable that corresponds to your controller.  You can disable this by adding 'prepopulate false' to your controller, like so:
<pre>
<code>
class ApplicationController < ActionController::Base
  include RestfulController
<em>  prepopulate false</em>
end
</code>
</pre>
